...
(defonce selected-side (r/atom nil))
...
(defn render-triangle [{[side1 side2 side3] :sides
                        [angle1 angle2 angle3] :angles
                        [label1 label2 label3] :vertices}
                       triangle]
  (let [rad1 (* angle1 (/ js/Math.PI 180))
        rad2 (* angle2 (/ js/Math.PI 180))
        place-side1 [0 0 0 side1]
        cy (/ (- (+ (sq side1) (sq side3)) (sq side2))
              (* 2 side1))
        cx (.sqrt js/Math (- (sq side3) (sq cy)))
        height (inc (max (inc side1) cy (inc (.abs js/Math (- cy side1)))))
        max-side (max side1 side2 side3)
        hovered (r/atom nil)]
      [:div [:svg {:width    "100%"
                   :view-box
                   (str (- (/ max-side 15)) " "
             ...
             [:g
              (when (= (/ pi 2) angle2)
                [right-angle-box 0 (- side1 (* height 0.065)) (/ height 16)])
              [:line {:x1 0 :x2 0 :y1 0 :y2 side1 :stroke-linecap "round"
                      :stroke (if (or (= @selected-side 1)
                      ...
                      :on-click #(reset! selected-side 1)}]
              [:line {:x1 0 :x2 cx :y1 side1 :y2 cy :stroke-linecap "round"
                      :stroke (if (or (= @selected-side 2)
                      ...
                      :on-click #(reset! selected-side 2)}]
              [:line {:x1 cx :x2 0 :y1 cy :y2 0 :stroke-linecap "round"
                      :stroke (if (or (= @selected-side 3)
                      ...
                      :on-click #(reset! selected-side 3)}]]
             [:g
              [latex/render-letter
              ...
              [latex/render-letter
               (keyword label2) (+ (- (/ max-side 20)) -0.1) side1 (/ height 16000)
             ...
             [:g ; enlarged click/hover targets for selecting angles
              ...
              [:circle {:cx (+ (- (/ max-side 20)) -0.1) :cy side1 :r (/ max-side 15)
